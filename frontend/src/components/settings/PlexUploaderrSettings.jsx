...
const PlexUploaderrSettings = ({ onDirtyChange }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        settings: {
          reapplyPosters: settings.reapplyPosters,
        },
      };
      const response = await fetch("/api/settings/save-plex-uploaderr", {
        method: "POST",
        headers: {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        const response = await fetch("/api/settings/get-plex-uploaderr");
        const result = await response.json();
        ...
        <div className="grid grid-cols-1 gap-4 border-b border-gray-500 py-5 lg:grid-cols-2">
          <label className="flex cursor-pointer items-start gap-3">
            <input
              ...
              onChange={(e) =>
                setSettings({ ...settings, reapplyPosters: e.target.checked })
              }
...
export default PlexUploaderrSettings;
